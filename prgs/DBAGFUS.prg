   ...
   IF lAcceptd
      ...
      IF lSavBef
         AEVAL( aValues, { | x1, i1 | aValues[ i1 ] := ;
                          _Getvalue( Num2Name( "cfc_", i1 ), "frmCombFrm" ) } )
      ENDIF lSavBef
...
FUNC ComboForm(;                          // Define a multiple values form ( Window )
   ...
   IF !EMPTY( aButtons )
       AEVAL( aButtons, { | a1, i1 | ASIZE( a1, 8 ) } )
       ...
       AEVAL( aButtons, { | a1, i1 | nButnAsCo += IF( a1[ 8 ] > 0, 1, 0 ) } )
...
FUNC CoFldList( ;                         // Conditional Field List
   ...
   AEVAL( aSelect, { | a1, i1 | cRVal += IF(aSelect[i1], FIELDNAME( i1 ) + ',' , '' ) } )
